...
@Api(value = "/User As Admin")
class AdminController @Inject() (val messagesApi: MessagesApi, system: ActorSystem, userService: IUserService, mailer: MailerClient, environment: Environment) extends api.ApiController {
        ...
        userService.findByEmail(user_email).flatMap {
          ...
          case None =>
            ...
            userService.insert(UserModel(Some(0), user_email, user_first_name, user_last_name, user_password, user_confirm_password,
              token, true, 1, new DateTime())).flatMap {
              case 1 =>
                ok(Json.obj { "message" -> "admin has already inserted into database!" })
