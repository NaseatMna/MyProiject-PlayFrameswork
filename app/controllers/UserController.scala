...
@Api(value = "/User As Members")
class UserController @Inject() (val messagesApi: MessagesApi, system: ActorSystem, userService: IUserService, mailer: MailerClient, environment: Environment) extends api.ApiController {
        ...
        userService.findByEmail(user_email).flatMap {
          ...
          case None =>
            ...
            userService.insert(UserModel(Some(0), user_email, user_first_name, user_last_name, user_password, user_confirm_password,
              token, false, 4, new DateTime())).flatMap {
              case _ =>
                val url = "http://localhost:9000/members/activeuser/" + token
                val email = Email(
